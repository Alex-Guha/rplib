...
- **Hard browser assumptions baked in, not injected.** `parser/storage.js` talks to `localStorage` directly. A library should take a persistence adapter, or not own persistence at all. (It now lives under `parser/` rather than `utils/` because the keys it writes — `abstractDefinitions`, `rootView` — are bundled-DSL specific; an app with a custom `resolveView` doesn't pull this file in.)
